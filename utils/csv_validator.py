import pandas as pd

class ValidatorCSV:

    def __init__(self, dataframe: pd.DataFrame):
        self._dataframe = dataframe.copy()

    def validate_columns(self, requerid_columns: list):
        missing_columns = [col for col in requerid_columns if col not in self._dataframe.columns]
        if missing_columns:
            raise ValueError(f"Faltan las siguiente columnas requeridas: {missing_columns}")
        return True

    def validate_no_nulls(self) -> bool:
        if self._dataframe.isnull().values.any():
            raise ValueError("El DataFrame contiene valores nulos")       
        return True

    def validate_column_types(self, columns_types: dict) -> bool:
        for column, expected_type in columns_types.items():
            if column in self._dataframe.columns:
                if not pd.api.types.is_dtype_equal(self._dataframe[column].dtype, expected_type):
                    raise ValueError(f"La columna '{column}' no es del tipo esperado: {expected_type}")
        return True